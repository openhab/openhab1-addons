...
public class JdbcDerbyDAO extends JdbcBaseDAO {
	...
	private void initSqlQueries() {
		...
		//Prevent error against duplicate time value (seldom): No powerful Merge found: http://www.codeproject.com/Questions/162627/how-to-insert-new-record-in-my-table-if-not-exists
	...
	public ItemsVO doCreateItemsTableIfNot(ItemsVO vo){
		//boolean tableExists = Yank.queryScalar(SQL_IF_TABLE_EXISTS.replace("#searchTable#", vo.getItemsManageTable().toUpperCase()), String.class, null) == null; 
		boolean tableExists = doIfTableExists(vo);
		if(tableExists){
			String sql = StringUtilsExt.replaceArrayMerge(
					SQL_CREATE_ITEMS_TABLE_IF_NOT, 
		...
		}else logger.debug("JDBC::doCreateItemsTableIfNot tableExists={}, did no CREATE TABLE", tableExists);
		return vo;
	}
	 ...
	 * @return
	 */
	private String histItemFilterQueryProvider(FilterCriteria filter, int numberDecimalcount, String table, String simpleName) {
		...
		if( "NUMBERITEM".equalsIgnoreCase(simpleName) && numberDecimalcount > -1){
			...
			queryString += "5 AS DECIMAL(31," + numberDecimalcount + "))"; //31 is DECIMAL max precision https://db.apache.org/derby/docs/10.0/manuals/develop/develop151.html
		}else{
