...
public class StringUtilsExt {
	...
	public static final String replaceArrayMerge(String str, String[] separate, String[] separators) {
		for(int i=0; i<separators.length; i++){
			str = str.replaceFirst((String) separate[i], (String) separators[i]);
		}
		return str;
	}
	 ...
	 * jdbc:dbShortcut://192.168.0.145:3306/databaseName?param1=false&param2=true<p/>
	 *
	 * @param url - JDBC-URI
	...
	public static Properties parseJdbcURL(String url, Properties def){
		...
		if(dbURI.getPath() != null){
			...
			if(dbURI.getScheme() != null && dbURI.getScheme().length() == 1) path = dbURI.getScheme() + ":" + path;
			props.put("serverPath", path);
		}
		if(dbURI.getPath() != null)props.put("databaseName", stringAfterLastSubstr(dbURI.getPath(), "/"));
		if(dbURI.getPort() != -1)props.put("portNumber", dbURI.getPort() + "");
		if(dbURI.getHost() != null)props.put("serverName", dbURI.getHost());
    ...
    public static void main(String[] args) throws Exception {
    	...
    	String[] u = new String[]{
    		"jdbc:postgresql://192.168.0.217:5432/lewienergy_gaubach",
	    	...
	    	"jdbc:dbShortcut:scheme://192.168.0.145:3306/databaseName",
	    	"jdbc:dbShortcut:scheme://192.168.0.145:3306/path/path/databaseName",
	    	"jdbc:dbShortcut:databaseName",
    	...
    	for(int i=0;i<u.length;i++){
        	System.out.printf("\n\n"+u[i]);
        	System.out.printf("\n"+StringUtilsExt.parseJdbcURL(u[i]));
    	}
    }
