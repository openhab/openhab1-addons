...
public class Db4oPersistenceService implements QueryablePersistenceService {
	...
	private void scheduleJob() {
		try {
			...
			// schedule backup-job
			JobDetail backupJob = newJob(CommitJob.class)
	...
	private void cancelAllJobs() {
		try {
			...
			if (jobKeys.size() > 0) {
				...
				logger.debug("Found {} Commit-Jobs to delete from DefaulScheduler (keys={})", jobKeys.size(), jobKeys);
			}
		} catch (SchedulerException e) {
	...
	public static class CommitJob implements Job {
		...
		public void execute(JobExecutionContext context) throws JobExecutionException {
			...
			try {
				ExtObjectContainer extObjectContainer = db.ext();
				System.err.println(extObjectContainer.version());
				db.commit();
				logger.debug("succesfully commited db4o transaction in {}ms", System.currentTimeMillis() - startTime);
			} catch(Db4oException e) {
				db.rollback();
