...
public class CULSerialHandlerImpl extends AbstractCULHandler implements SerialPortEventListener {
            ...
            try {
                if (isValidParity(configuredParity)) {
                    ...
                    if (isValidParity(parsedParityNumber)) {
                        ...
                        log.error("The configured '{}' value is invalid. The value '{}' has to be one of {}.",
    ...
    private Integer baudrateFromConfig(final String configuredBaudRate) {
        if (StringUtils.isNotBlank(configuredBaudRate)) {
            try {
                ...
                log.error("Error parsing config parameter '{}' to integer. Value = {}", KEY_BAUDRATE,
                        configuredBaudRate);
            }
    ...
    public boolean arePropertiesEqual(Map<String, ?> properties) {
        ...
        if (properties.containsKey(KEY_PARITY)) {
            ...
            if (!configured.equals(parityMode)) {
                return false;
            }
        }
     ...
     */
    private boolean mapContainsEqualValueByKey(Map<String, ?> properties, String key, Integer value) {
        boolean result = true;
