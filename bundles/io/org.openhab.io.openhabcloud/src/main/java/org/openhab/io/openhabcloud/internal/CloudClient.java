...
/**
 * This class provides communication between openHAB and openHAB Cloud service.
 * It also implements async http proxy for serving requests from user to
 * openHAB through openHAB Cloud. It uses Socket.IO connection to connect to
 * openHAB Cloud service and Jetty Http client to send local http requests to
 * openHAB.
 *
 * @author Victor Belov
 * @since 1.3.0
...
public class CloudClient {
    ...
    // openHAB UUID
    /*
     * This variable holds openHAB's UUID for authenticating and connecting to openHAB Cloud cloud
     */
    private String mUUID;
    /*
     * This variable holds openHAB's secret for authenticating and connecting to openHAB Cloud cloud
     */
     ...
     */
    private HashMap<Integer, OHCloudExchange> mRunningRequests;
    /*
     * This variable indicates if connection to openHAB Cloud cloud is currently in an established state
     */
     ...
     * certain events from openHAB Cloud cloud back to openHAB
     */
    private CloudClientListener mListener;
     ...
     * 
     * @param uuid openHAB's UUID to connect to openHAB Cloud
     * @param secret openHAB's Secret to connect to openHAB Cloud
     * 
     */

    public CloudClient(String uuid, String secret, String baseURL, boolean remoteAccessEnabled, Set<String> exposedItems) {
        mUUID = uuid;
        mSecret = secret;
        this.baseURL = baseURL;
        this.remoteAccessEnabled = remoteAccessEnabled;
        this.exposedItems = exposedItems;
        mRunningRequests = new HashMap<Integer, OHCloudExchange>();
    }

    /**
     * Connect to openHAB Cloud
     */
            ...
            public void call(Object... args) {
                ...
                transport.on(Transport.EVENT_REQUEST_HEADERS, new Emitter.Listener() {
                    ...
                    public void call(Object... args) {
                        ...
                        headers.put("clientversion", Arrays.asList(CloudServiceImpl.myohVersion));
    ...
    private void stopJetty() {
        if (mJettyClient != null) {
            try {
                ...
                logger.error("Error stopping JettyClient: {}", e.getMessage());
            }
        }
    }
    ...
    public void onConnect() {
        logger.info("Connected to the openHAB Cloud service (UUID = {}, local base URL = {})", this.mUUID, this.mOHBaseUrl);
        mIsConnected = true;
    ...
    public void onDisconnect() {
        logger.info("Disconnected from the openHAB Cloud service (UUID = {}, local base URL = {})", this.mUUID,
                this.mOHBaseUrl);
        mIsConnected = false;
    ...
    private void handleRequestEvent(JSONObject data) {
        try {
            ...
            OHCloudExchange exchange = new OHCloudExchange(requestId);
    ...
    private void handleCancelEvent(JSONObject data) {
        try {
            ...
            if (mRunningRequests.containsKey(requestId)) {
                OHCloudExchange requestExchange = mRunningRequests.get(requestId);
    ...
    /**
     * This method sends notification to openHAB Cloud
     * 
    ...
    /**
     * This method sends log notification to openHAB Cloud
     * 
     * @param message notification message text
     * @param icon name of the icon for this notification
     * @param severity severity name for this notification
    ...
    /**
     * This method sends broadcast notification to openHAB Cloud
     * 
     * @param message notification message text
     * @param icon name of the icon for this notification
     * @param severity severity name for this notification
    ...
    public void setListener(CloudClientListener mListener) {
        this.mListener = mListener;
    }

    /*
     * An internal class which extends ContentExchange and forwards response
     * headers and data back to openHAB Cloud
     * 
     */

    private class OHCloudExchange extends ContentExchange {

        private int mRequestId;
        private HashMap<String, String> mResponseHeaders;

        public OHCloudExchange(int requestId) {
