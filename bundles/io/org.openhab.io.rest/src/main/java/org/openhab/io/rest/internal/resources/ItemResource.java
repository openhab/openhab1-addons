...
public class ItemResource {
	...
	@GET @Path("/alarmed") 
	@Produces( { MediaType.WILDCARD })
	public Response getAlarmedItems(
			@Context HttpHeaders headers,
			@QueryParam("type") String type, 
			@QueryParam("jsoncallback") @DefaultValue("callback") String callback) {
		logger.debug("Received HTTP GET request at '{}' for media type '{}'.", new Object[] { uriInfo.getPath(), type });
		...
		if(responseType!=null) {
			Object responseObject = responseType.equals(MediaTypeHelper.APPLICATION_X_JAVASCRIPT) ?
					new JSONWithPadding(new ItemListBean(getAlarmedItemBeans()), callback) : new ItemListBean(getAlarmedItemBeans());
					return Response.ok(responseObject, responseType).build();
		} else {
			return Response.notAcceptable(null).build();
		}
	...
	@GET @Path("/{itemname: [a-zA-Z_0-9]*}/alarm") 
	@Produces( { MediaType.TEXT_PLAIN })
	public SuspendResponse<String> getPlainItemAlarmState(
			@PathParam("itemname") String itemname, 
			@HeaderParam(HeaderConfig.X_ATMOSPHERE_TRANSPORT) String atmosphereTransport,
			@Context AtmosphereResource resource) {
		if(atmosphereTransport==null || atmosphereTransport.isEmpty()) {
	...
	@GET @Path("/{itemname: [a-zA-Z_0-9]*}/alarmtext") 
	@Produces( { MediaType.TEXT_PLAIN })
	public SuspendResponse<String> getPlainItemAlarmText(
			@PathParam("itemname") String itemname, 
			@HeaderParam(HeaderConfig.X_ATMOSPHERE_TRANSPORT) String atmosphereTransport,
			@Context AtmosphereResource resource) {
		if(atmosphereTransport==null || atmosphereTransport.isEmpty()) {
	...
	@GET @Path("/{itemname: [a-zA-Z_0-9]*}/alarmclass") 
	@Produces( { MediaType.TEXT_PLAIN })
	public SuspendResponse<String> getPlainItemAlarmClass(
			@PathParam("itemname") String itemname, 
			@HeaderParam(HeaderConfig.X_ATMOSPHERE_TRANSPORT) String atmosphereTransport,
			@Context AtmosphereResource resource) {
		if(atmosphereTransport==null || atmosphereTransport.isEmpty()) {
	...
	@GET @Path("/{itemname: [a-zA-Z_0-9]*}/alarmtime_utc") 
	@Produces( { MediaType.TEXT_PLAIN })
	public SuspendResponse<String> getPlainItemAlarmTime(
			@PathParam("itemname") String itemname, 
			@HeaderParam(HeaderConfig.X_ATMOSPHERE_TRANSPORT) String atmosphereTransport,
			@Context AtmosphereResource resource) {
		if(atmosphereTransport==null || atmosphereTransport.isEmpty()) {
	...
	public static ItemBean createItemBean(Item item, boolean drillDown, String uriPath) {
		...
		bean.alarm = String.valueOf(item.isAlarmed());
		if ((item.getAlarmState()==null)|| (item.getAlarmState().getAlarmMessage()==null)) {
			bean.alarmtext = "";
			bean.alarmclass = "";
			bean.alarmtime_utc = "";
		}
		else {
			bean.alarmtext = item.getAlarmState().getAlarmMessage();
			bean.alarmclass = item.getAlarmState().getAlarmClass().toString();
			bean.alarmtime_utc = String.valueOf(item.getAlarmState().getAlarmTimeUTC());
		}
	...
	/**
	 * Returns a list of ItemBeans for alarmed items or an empty list
	 * if there are no alarmed items.
	 * @return a list of ItemBeans for alarmed items or empty list 
	 */
	private List<ItemBean> getAlarmedItemBeans() {
		List<ItemBean> beans = new LinkedList<ItemBean>();
		ItemUIRegistry registry = RESTApplication.getItemUIRegistry();
		for(Item item : registry.getItems()) {
			if (item.isAlarmed()) {
