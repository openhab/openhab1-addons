 ...
 * Interface. To be callable via the {@link TransformationService} the <code>component.name</code>
 * property should be set accordingly. For example if one provides a new processor
 * with the component name <code>'org.openhab.core.transform.processor.foo'</code>
 * then this processor can be addressed via the function name <code>'foo()'</code> 
 * 
