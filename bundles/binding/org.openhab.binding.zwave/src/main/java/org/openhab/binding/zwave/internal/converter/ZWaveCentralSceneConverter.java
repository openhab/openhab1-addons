...
import org.openhab.binding.zwave.internal.converter.state.*;
...
public class ZWaveCentralSceneConverter extends ZWaveCommandClassConverter<ZWaveSceneActivationCommandClass> {
    ...
    /**
     * Constructor. Creates a new instance of the {@link ZWaveCentralSceneConverter}
     * class.
     *
    ...
    void handleEvent(ZWaveCommandClassValueEvent event, Item item, Map<String, String> arguments) {
        ...
        if (arguments.get("scene") != null) {
            ...
            // ensure the item specified scene number matches the scene number from the received command event
            if (scene != null && scene != event_scene) {
                return;
            }

            // if the optional "key" argument was defined in the item definition,
            ...
            if (arguments.get("key") != null) {
                ...
                // ensure the item specified key number matches the key number from the received command event
                ...
                // convert the scene number and key state to a binary value on a successful match
                ...
                // convert the scene's key number to an acceptable conversion type and then publish the event
                itemState = converter.convertFromValueToState(event_key);
            }
        }
        else{
            // no scene argument provide, so we will convert the central scene number
