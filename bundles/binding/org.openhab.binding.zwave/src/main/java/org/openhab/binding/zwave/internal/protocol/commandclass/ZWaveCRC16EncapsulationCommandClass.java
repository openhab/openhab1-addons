...
public class ZWaveCRC16EncapsulationCommandClass extends ZWaveCommandClass {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ZWaveCRC16EncapsulationCommandClass.class);
	 ...
	 */
	public ZWaveCRC16EncapsulationCommandClass(ZWaveNode node,
	...
	public void handleApplicationCommandRequest(SerialMessage serialMessage,
		...
		logger.debug("NODE {}: Received CRC 16 Encapsulation Request", this
				.getNode().getNodeId());
	 ...
	 */
	private void handleCRC16EncapResponse(SerialMessage serialMessage,
			int offset) {
		logger.trace("Process CRC16 Encapsulation");

		// Check CRC
		byte[] payload = serialMessage.getMessagePayload();
		byte[] messageCrc = Arrays.copyOfRange(payload, payload.length - 2,
				payload.length);
		byte[] tocheck = Arrays.copyOfRange(payload, offset - 2,
				payload.length - 2);
		...
		if (commandClass == null) {
			logger.error(String.format(
					"NODE %d: Unsupported command class 0x%02x", this.getNode()
							.getNodeId(), commandClassCode));
		} else {
			...
			if (zwaveCommandClass == null) {
				logger.debug(String
						.format("NODE %d: Command class %s (0x%02x) not found, trying to add it.",
								getNode().getNodeId(), commandClass.getLabel(),
								commandClass.getKey()));

				zwaveCommandClass = ZWaveCommandClass.getInstance(
						commandClass.getKey(), getNode(), getController());

				if (zwaveCommandClass != null) {
					logger.debug(String.format(
							"NODE %d: Adding command class %s (0x%02x)",
							getNode().getNodeId(), commandClass.getLabel(),
							commandClass.getKey()));
			...
			if (zwaveCommandClass == null) {
				...
				logger.debug(String.format(
						"NODE %d: Calling handleApplicationCommandRequest.",
						this.getNode().getNodeId()));
				zwaveCommandClass.handleApplicationCommandRequest(
						serialMessage, offset+1, 0); //TODO get endpoint from somewhere
			}
		}
	}
