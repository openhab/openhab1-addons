 ...
 */
package org.openhab.binding.netatmowelcome.internal.messages;

import java.util.Properties;

import org.codehaus.jackson.map.JsonMappingException;
import org.codehaus.jackson.map.ObjectMapper;
import org.openhab.binding.netatmowelcome.internal.NetatmoWelcomeException;
...
public abstract class AbstractRequest extends AbstractMessage implements Request {
	...
	protected final RuntimeException newException(
		...
		if(cause instanceof JsonMappingException) {
			return new NetatmoWelcomeException("Could not parse JSON from URL '"
					+ url + "' content='" + content + "' json='" + json
					+ "'", cause);
		}

		return new NetatmoWelcomeException(message, cause);
	}
