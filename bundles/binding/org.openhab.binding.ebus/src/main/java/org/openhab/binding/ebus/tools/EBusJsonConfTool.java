...
public class EBusJsonConfTool {
    ...
    static String ALLOWED_LEVEL_1_PARAMS[] = { "comment", "device", "id", "class", "command", "data", "values",
            "computed_values", "debug" };
        ...
        for (Map<String, ?> entry : configurationMap) {
            ...
            for (String valueKey : valueEntries.keySet()) {
                ...
                if (!map.containsKey("hide") && !StringUtils.startsWith(valueKey, "_")) {
                    ...
                    t = t.equalsIgnoreCase("bit") ? "Switch"
                            : t.equalsIgnoreCase("script") ? "???" : t.equalsIgnoreCase("string") ? "Text" : "Number";
                    t = "``" + t + "``";
                    line.add(t);
    ...
    private void checkConfiguration(List<Map<String, ?>> configurationMap) {
        ...
        for (Map<String, ?> entryMap : configurationMap) {
            ...
            String unid = (String) (entryMap.containsKey("command")
                    ? entryMap.get("command") + " XX " + entryMap.get("data") : entryMap.get("filter"));

            if (doubleEntries.containsKey(unid)) {
                sb.append("Combination of command and data already existing [" + doubleEntries.get(unid) + "]... "
                        + unid + "\n");
            } else {
