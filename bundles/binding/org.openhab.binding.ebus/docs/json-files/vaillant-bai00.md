...
**boiler.set_blocktime_boiler**        | ``Number`` | Max. burner anti-cycling time heating at 20 °C flow temperature (min)
**boiler.set_level_part_load**         | ``Number`` | Heating partial load (kW)                                            
**boiler.set_postrun_pump**            | ``Number`` | Overrun time of internal pump for heating mode (min)                 
**boiler.set_state_return_regulation** | ``Number`` | Heating flow/return regulation changeover - {0=flow, 1=return}       
