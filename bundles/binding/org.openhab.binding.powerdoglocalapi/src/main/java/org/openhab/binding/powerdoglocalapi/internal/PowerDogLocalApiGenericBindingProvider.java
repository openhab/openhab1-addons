...
public class PowerDogLocalApiGenericBindingProvider extends
		...
		PowerDogLocalApiBindingProvider {

	static final Logger logger = LoggerFactory
			.getLogger(PowerDogLocalApiGenericBindingProvider.class);

	/** {@link Pattern} which matches a binding configuration part */
	private static final Pattern BASE_CONFIG_PATTERN = Pattern
			.compile("(<|>)([0-9._a-zA-Z]+:[0-9._a-zA-Z:]+)");
	...
	protected static final Command IN_BINDING_KEY = StringType
			.valueOf("IN_BINDING");
	protected static final Command OUT_BINDING_KEY = StringType
			.valueOf("OUT_BINDING");
	...
	public void validateItemType(Item item, String bindingConfig)
		...
		if (!(item instanceof SwitchItem || item instanceof DimmerItem
				|| item instanceof ContactItem || item instanceof NumberItem || item instanceof StringItem)) {
	 ...
	 * @throws BindingConfigParseException
	 */
	protected PowerDogLocalApiBindingConfig parseBindingConfig(Item item,
			String bindingConfig) throws BindingConfigParseException {
		...
		if (!matcher.matches()) {
			throw new BindingConfigParseException(
					"PowerDogLocalAPI:bindingConfig '" + bindingConfig
		...
		while (matcher.find()) {
			...
			if (direction.equals("<")) {
				...
				config = parseInnerBindingConfig(item, bindingConfigPart,
						config, IN_BINDING_KEY);
			}
			//
			else if (direction.equals(">")) {
				// out-binding config line found
				config = parseInnerBindingConfig(item, bindingConfigPart,
						config, OUT_BINDING_KEY);
			} else {
				throw new BindingConfigParseException(
	 ...
	 */
	protected PowerDogLocalApiBindingConfig parseInnerBindingConfig(Item item,
			String bindingConfig, PowerDogLocalApiBindingConfig config,
			Command key) throws BindingConfigParseException {
	...
	public Class<? extends Item> getItemType(String itemName) {
		PowerDogLocalApiBindingConfig config = (PowerDogLocalApiBindingConfig) bindingConfigs
				.get(itemName);
	...
	public List<String> getInBindingItemNames() {
		...
		for (String itemName : bindingConfigs.keySet()) {
			PowerDogLocalApiBindingConfig pdConfig = (PowerDogLocalApiBindingConfig) bindingConfigs
					.get(itemName);
