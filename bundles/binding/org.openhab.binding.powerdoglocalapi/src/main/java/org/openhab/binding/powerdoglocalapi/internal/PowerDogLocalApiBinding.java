...
public class PowerDogLocalApiBinding extends
	...
	private static final Logger logger = LoggerFactory
			.getLogger(PowerDogLocalApiBinding.class);
		...
		for (PowerDogLocalApiBindingProvider provider : providers) {
			...
			for (String itemName : provider.getInBindingItemNames()) {
				...
				if (itemNeedsUpdate) {
					...
					XmlRpcStruct response = loadPowerDogResponse(
							serverNeedsUpdate, server);

					// update item state
					if (response != null) {
						String value = getVariable(response,
								provider.getValueId(itemName),
	...
	private XmlRpcStruct loadPowerDogResponse(boolean needsUpdate,
			PowerDogLocalApiServerConfig server) {
		...
		if (needsUpdate == true) {
			try {
				...
				PowerDog powerdog = (PowerDog) XmlRpcProxy
						.createProxy(server.url(), "",
	 ...
	 * @return
	 */
	private String getVariable(XmlRpcStruct response, String valueId,
			String name) {
		try {
	...
	protected void internalReceiveUpdate(String itemName, State newState) {
		...
		for (PowerDogLocalApiBindingProvider provider : providers) {
			...
			if (provider.getOutBindingItemNames().contains(itemName)) {
				...
				long age = System.currentTimeMillis() - lastUpdateTimeStamp;
				boolean itemMayUpdate = (age >= provider
						.getRefreshInterval(itemName));
				...
				if (itemMayUpdate) {
					...
					try {
						...
						PowerDog powerdog = (PowerDog) XmlRpcProxy.createProxy(
								server.url(), "",
								new Class[] { PowerDog.class }, false);
						XmlRpcStruct response = powerdog.setLinearSensorDevice(
								server.password, provider.getValueId(itemName),
								value);
	...
	private void parseConfiguration(Map<String, Object> config) {
		...
		if (config != null) {
			...
			for (Iterator<String> keys = keyset.iterator(); keys.hasNext();) {
				...
				PowerDogLocalApiServerConfig deviceConfig = serverList
						.get(serverId);
	...
	static class PowerDogLocalApiServerConfig {
		...
		public String toString() {
			...
			return "PowerDogLocalApiServerCache [host=" + host + ", password="
					+ displayPassword + ", lastUpdate=" + lastUpdate
					+ ", cache=" + cache + "]";
		}
	...
	static interface PowerDog {
		...
		public XmlRpcStruct getLinearDevices(String password)
				throws XmlRpcFault;

		public XmlRpcStruct getAllCurrentLinearValues(String password)
				throws XmlRpcFault;

		public XmlRpcStruct getCurrentLinearValues(String password,
				String comma_seperated_list_of_keys) throws XmlRpcFault;

		public XmlRpcStruct setLinearSensorDevice(String password, String key,
				String current_value) throws XmlRpcFault;

		public XmlRpcStruct setLinearCounterDevice(String password, String key,
				String current_value, String countup_meter_reading)
				throws XmlRpcFault;
	}
}
