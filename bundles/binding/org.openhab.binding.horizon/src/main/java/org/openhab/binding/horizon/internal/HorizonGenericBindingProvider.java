...
import nl.kuijp.horizoncontrol.Keys;
...
public class HorizonGenericBindingProvider extends AbstractGenericBindingProvider implements HorizonBindingProvider {
	...
	private void parseBindingConfig(String bindingConfigs,
			HorizonBindingConfig config) throws BindingConfigParseException {
		String bindingConfig = StringUtils.substringBefore(bindingConfigs, ",");
		String bindingConfigTail = StringUtils.substringAfter(bindingConfigs, ",");
		String[] configParts = bindingConfig.trim().split(":");
		if (configParts.length != 3) {
			throw new BindingConfigParseException(
					"Horizon binding must contain three parts separated by ':'");
		}
		...
		Integer key = Keys.getByName(horizonCommand);
