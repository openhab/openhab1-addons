...
public enum BenqProjectorItemMode {
	...
	POWER("pow", ResponseType.ON_OFF), MUTE("mute", ResponseType.ON_OFF), VOLUME(
			"vol", ResponseType.NUMBER), LAMP_HOURS("ltim", ResponseType.NUMBER), SOURCE_STRING(
			"sour", ResponseType.STRING), SOURCE_NUMBER("sour",
			ResponseType.SOURCE_MAPPING);

	public enum ResponseType {
		ON_OFF, NUMBER, STRING, SOURCE_MAPPING;
	}
	...
	private State parseStringResponse(String response) {
		String[] responseParts = response.split("=");
		if (responseParts.length == 2) {
			return new StringType(responseParts[1].substring(0,
					responseParts[1].length() - 1));
		}
		return UnDefType.UNDEF;
	}
		...
		case SOURCE_MAPPING:
			...
			if (s instanceof StringType) {
				...
				return new DecimalType(
						BenqProjectorSourceMapping.getMappingFromString(strT
								.toString()));
			} else
				return s;
		case NUMBER:
