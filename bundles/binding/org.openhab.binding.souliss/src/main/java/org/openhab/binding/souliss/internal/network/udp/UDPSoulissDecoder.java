...
public class UDPSoulissDecoder {
    ...
    private void decodeMacaco(ArrayList<Short> macacoPck) {
        ...
        switch (functionalCode) {
            ...
            case ConstantsUDP.Souliss_UDP_function_ping_resp:
                logger.info("function_ping_resp");
                decodePing(macacoPck);
                break;
            case ConstantsUDP.Souliss_UDP_function_subscribe_resp:
            case ConstantsUDP.Souliss_UDP_function_poll_resp:
            ...
            case ConstantsUDP.Souliss_UDP_function_typreq_resp:// Answer for
                ...
                break;
            case ConstantsUDP.Souliss_UDP_function_health_resp:// Answer
                                                               // nodes
                                                               // healty
    ...
    private void decodeTypRequest(ArrayList<Short> mac) {
        try {
            ...
            logger.info("--DECODE MACACO OFFSET: {} NUMOF: {} TYPICALSXNODE: {}", tgtnode, numberOf, typXnodo);
            ...
            while (iteratorTypicals.hasNext()) {
                ...
                if (typ.getSoulissNodeID() == tgtnode) {
                    ...
                    if (typ.getType() != 152 && typ.getType() != 153) {
                        if (iNumBytes == 4) {
                            // RGB Log
                            logger.debug("decodeStateRequest: {} ({}) = {}. RGB= {}, {}, {}", typ.getName(),
                                    Short.valueOf(typ.getType()), ((SoulissT16) typ).getState(),
                                    ((SoulissT16) typ).getStateRED(), ((SoulissT16) typ).getStateGREEN(),
                                    ((SoulissT16) typ).getStateBLU());
                        } else if (iNumBytes == 5) {
                            // T31 Thermostat
                            logger.debug(
                                    "decodeStateRequest: {} ({}). Thermostat= {}, Temp.Measured= {}, Temp.SetPoint= {}",
                                    typ.getName(), Short.valueOf(typ.getType()),
                            ...
                            if (typ.getType() == 0x1A) {
                                logger.debug("decodeStateRequest: {} (0x{}) = {}", typ.getName(),
                                        Integer.toHexString(typ.getType()),
                                        Integer.toBinaryString(((SoulissT1A) typ).getRawState()));
                            } else {
                                logger.debug("decodeStateRequest: {} (0x{}) = {}", typ.getName(),
                                        Integer.toHexString(typ.getType()), Float.valueOf(val));
                            }
                        }
                    }
                }
