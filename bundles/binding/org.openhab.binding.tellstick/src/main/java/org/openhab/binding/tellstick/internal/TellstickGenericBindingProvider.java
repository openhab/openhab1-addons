 ...
 * @since 1.5.0
 */
public class TellstickGenericBindingProvider extends AbstractGenericBindingProvider
        implements TellstickBindingProvider {
    ...
    private void validateBinding(Item item, String[] configParts, TellstickDevice device)
        ...
        if (device == null && !StringUtils.isNumeric(configParts[0].trim())) {
            throw new BindingConfigParseException(
                    "item '" + item.getName() + "' telldus device " + configParts[0].trim() + " not found");
        }
    }
