 ...
 * @since 1.3.0
 */
public class DigitalSTROMBinding extends AbstractBinding<DigitalSTROMBindingProvider> implements ItemListener {
	...
	public void deactivate() {
		for (DigitalSTROMBindingProvider provider : providers) {
			provider.removeItemListener(this);
			provider.shutdownBinding();
	...
	protected void internalReceiveCommand(String itemName, Command command) {
		for (DigitalSTROMBindingProvider provider: providers) {
			provider.sendCommandToDSS(itemName, command);
		}
	}
