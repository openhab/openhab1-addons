...
/**
 * This function parses the Binding Configuration for Max! CUL bindings
 * and populates the configuration object
 *
...
public class MaxCulBindingConfigParser {
	...
	private static final Logger logger =
			LoggerFactory.getLogger(MaxCulBindingConfigParser.class);

	public static void  parseMaxCulBindingString( String bindingConfigStr, MaxCulBindingConfig cfg ) throws BindingConfigParseException
	{
		...
		else
		{
			...
			/* load stored configuration from pairing (if present) except on pair/listen mode bindings */
			if (cfg.getDeviceType() != MaxCulDevice.PAIR_MODE || cfg.getDeviceType() != MaxCulDevice.LISTEN_MODE )
				cfg.loadStoredConfig();
		}
	}

	private static void parseDeviceCategory(String configPart, MaxCulBindingConfig cfg) throws BindingConfigParseException
	{
	...
	private static void parseDeviceFeature(String configParts, MaxCulBindingConfig cfg) throws BindingConfigParseException
	{
		...
		if (configPartArray.length == 2)
		{
			...
			if (configPart.equals("thermostat")) {
				if (cfg.getDeviceType() != MaxCulDevice.RADIATOR_THERMOSTAT
					...
					throw new BindingConfigParseException(
							"Invalid device feature. Can only use 'thermostat' on radiator or wall thermostats. This is a "+cfg.getDeviceType());
				cfg.setFeature(MaxCulFeature.THERMOSTAT);
				...
				if (cfg.getDeviceType() != MaxCulDevice.RADIATOR_THERMOSTAT
						...
						&& cfg.getDeviceType() != MaxCulDevice.WALL_THERMOSTAT)
					throw new BindingConfigParseException(
							"Invalid device feature. Can only use 'temperature' on radiator or wall thermostats. This is a "+cfg.getDeviceType());
				...
				if (cfg.getDeviceType() != MaxCulDevice.PUSH_BUTTON
					...
					throw new BindingConfigParseException(
							"Invalid device feature. Can only use 'switch' on PushButton or ShutterContact. This is a {}"+cfg.getDeviceType());
				cfg.setFeature(MaxCulFeature.TEMPERATURE);
			} else if (configPart.equals("valvepos")) {
				if (cfg.getDeviceType() != MaxCulDevice.RADIATOR_THERMOSTAT
						&& cfg.getDeviceType() != MaxCulDevice.RADIATOR_THERMOSTAT_PLUS)
								throw new BindingConfigParseException(
										"Invalid device feature. Can only use 'switch' on RadiatorThermostat or RadiatorThermostatPlus. This is a "+cfg.getDeviceType());
	...
	private static void parseAssociation(String configPart, MaxCulBindingConfig cfg) throws BindingConfigParseException
	{
		String[] configKeyValueSplit = configPart.split("=");
		if (configKeyValueSplit.length == 2)
		{
	 ...
	 * @param config Populated configuration to check
	 * @param item Item to check
	 * @throws BindingConfigParseException Thrown when item type is invalid
	 */
