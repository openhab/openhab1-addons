...
public class WallThermostatStateMsg extends BaseMsg {
	...
	final static private int WALL_THERMOSTAT_STATE_PAIR_THERM_PAYLOAD_LEN = 5; /* in bytes - same as above but also includes paired thermostat measured temp */
	final static private int WALL_THERMOSTAT_STATE_UNTIL_PAYLOAD_LEN = 6; /* in bytes - same as above but includes time until */
	final static private int WALL_THERMOSTAT_STATE_TEMP_PAYLOAD_LEN = 7; /* in bytes - same as above but includes measured temperature as well */


	private static final Logger logger =
			LoggerFactory.getLogger(WallThermostatStateMsg.class);
	...
	public WallThermostatStateMsg(String rawMsg) {
				...
				this.payload.length == WALL_THERMOSTAT_STATE_TEMP_PAYLOAD_LEN)
		{
			...
			/* extract whether wall therm is configured to show measured or desired temperature */
			...
			else if (this.payload.length ==WALL_THERMOSTAT_STATE_TEMP_PAYLOAD_LEN)
			{
				...
				this.measuredTemperature = mTemp/10.0;
				// TODO extract Date/Time if payload is WALL_THERMOSTAT_STATE_TEMP_PAYLOAD_LEN
				// Current not used
			}
		}
		else {
