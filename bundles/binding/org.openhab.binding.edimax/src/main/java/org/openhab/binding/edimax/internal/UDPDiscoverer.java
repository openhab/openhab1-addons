...
public class UDPDiscoverer implements Discoverer {
	 ...
	 * Discovery Package.
	 */
	byte[] DISCOVERY_BYTES = { (byte) 0xff, (byte) 0xff, (byte) 0xff,
			(byte) 0xff, (byte) 0xff, (byte) 0xff, 0x45, 0x44, 0x49, 0x4d,
			0x41, 0x58, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, (byte) 0xa1,
			(byte) 0xff, 0x5e };

	private EdimaxDevice[] discover() throws SocketException,
		...
		try {
			...
			DatagramPacket sendPacket = new DatagramPacket(sendData,
					sendData.length, ipAddress, 20560);
			...
			try {
				while (true) {
					...
					if (!StringUtils.isEmpty(sentence)
							&& sentence.contains("EDIMAX")) {
						byte[] mac = new byte[6];
						System.arraycopy(receivePacket.getData(), 0, mac, 0, 6);

						String encodedMAC = Hex.encodeHexString(mac)
								.toUpperCase();
		...
		return discoveredDevices.toArray(new EdimaxDevice[discoveredDevices
				.size()]);
	}
