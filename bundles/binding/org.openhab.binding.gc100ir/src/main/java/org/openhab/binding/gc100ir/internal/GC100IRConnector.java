...
public class GC100IRConnector {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(GC100IRConnector.class);
	 ...
	 */
	public GC100IRConnector(GC100IRHost gc100, EventPublisher eventPublisher) {
		...
		try {
			deviceResponse = this.gc100ControlPoint.queryGC100(
					GC100IRControlPoint.GC_100_QUERY_MESSAGE_GET_DEVICES,
					gc100.getHostname());
		...
		itemUtility = ItemUtility.getInstance();
	}
	/**
	...
	public boolean ping() {
		...
		HashMap<String, GC100ItemBean> items = itemUtility.getAllItems();
		if (!items.isEmpty()) {
			for (String itemName : items.keySet()) {
				GC100ItemBean itemBean = items.get(itemName);
				if (!gc100ControlPoint.connectTarget(gc100.getHostname(),
						itemBean.getModule(), itemBean.getConnector())) {
					logger.warn("Problem in connecting " + gc100.getHostname()
							+ " GC100 Module: " + itemBean.getModule()
							+ " of Connector: " + itemBean.getConnector());
				}
			}
		}
	...
	public void open() throws IOException, InterruptedException,
		...
		if (!items.isEmpty()) {
			for (String itemName : items.keySet()) {
				...
				if (!gc100ControlPoint.connectTarget(gc100.getHostname(),
						itemBean.getModule(), itemBean.getConnector())) {
					logger.warn("Problem in connecting host"
							+ gc100.getHostname() + " with item: " + itemName
							+ " of module: " + itemBean.getModule()
							+ " connector: " + itemBean.getConnector());
				}
			}
		}
		if (gc100ControlPoint.isGC100ItemConnected(gc100.getHostname()))
	...
	public void close() {
		...
		if (!items.isEmpty()) {
			for (String itemName : items.keySet()) {
				GC100ItemBean itemBean = items.get(itemName);
				if (gc100ControlPoint.isGC100ItemConnected(gc100.getHostname())) {
					gc100ControlPoint.disConnectTarget(gc100.getHostname(),
							itemBean.getModule(), itemBean.getConnector());
					logger.warn("Closing connection with item: " + itemName
							+ " of module: " + itemBean.getModule()
							+ " connector: " + itemBean.getConnector());
				}
			}
	...
	public GC100ItemBean getGC100ItemBean(String itemName) {
		return itemUtility.getAllItems().get(itemName);
	}
	/**
	...
	public void invokeCommand(String itemName) {
		...
		String preparedCode = itemUtility.prepareCode(itemBean);
		String result = gc100ControlPoint.doAction(gc100.getHostname(),
				itemBean.getModule(), itemBean.getConnector(), preparedCode);
		if (!result.contains("complete")) {
			logger.warn("Failed to execute command over GC100: "
					+ gc100.getHostname() + " with Module: "
					+ itemBean.getModule() + " of Connector: "
					+ itemBean.getConnector() + " to Item: " + itemName);
			return;
		}
		logger.info("Successfully executed command over GC100: "
				+ gc100.getHostname() + " with Module: " + itemBean.getModule()
				+ " of Connector: " + itemBean.getConnector() + " to Item: "
				+ itemName);
	}
}
