...
public class Cardio2eLoginTransaction extends Cardio2eTransaction {
	...
	public Cardio2eLoginTransaction() { //Simple logout constructor (this transaction is obsolete: provided for legacy compatibility).
		super.setObjectType (Cardio2eObjectTypes.LOGIN);
	}
		
	public Cardio2eLoginTransaction(Cardio2eLoginCommands logoutCommand) { // Simple set logout constructor (logout command is obsolete: only provided for legacy compatibility).
	...
	public Cardio2eLoginTransaction(String programCode) { //Simple set login constructor.
		setTransactionType (Cardio2eTransactionTypes.SET);
		super.setObjectType (Cardio2eObjectTypes.LOGIN);
	...
	public void setTransactionType(Cardio2eTransactionTypes transactionType) {
		if (transactionType != Cardio2eTransactionTypes.GET) {
			...
			throw new IllegalArgumentException("invalid transactionType '" + transactionType + "for a LOGIN transaction");
		}
	}
	...
	public String toString () {
		...
		if (isDataVerified()) {
			...
			case INFORMATION: //Signals Login Information End (@I P E)
				returnString = String.format("%s%s %s E%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,CARDIO2E_END_TRANSACTION_CHARACTER);
				break;
			case SET:
				if (programCode != null)
				{
					returnString = String.format("%s%s %s %s %s%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,loginCommand.symbol,programCode,CARDIO2E_END_TRANSACTION_CHARACTER);
				}
				else
				{
					returnString = String.format("%s%s %s %s%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,loginCommand.symbol,CARDIO2E_END_TRANSACTION_CHARACTER);
				}
				break;
			}			
		}
	...
	public boolean isSmartSendingEnabled() { //Returns true because this class would be always smartSending enabled.
		return true;	
	}
}
