...
public class Cardio2eLightingTransaction extends Cardio2eTransaction{
	...
	private byte lightIntensity = -1; // -1=Not set, signaling lightIntensity not set.
	public boolean lightIntensityCorrection = false; // If true, getLightIntensity() returns 0 when lightIntensity = 1 (Cardio 2é DM1 modules consider light is off for lower values of 2, because system sets DM1 modules light intensity value to 1% after auto lighting by presence ends). 

	public Cardio2eLightingTransaction() {
		super.setObjectType (Cardio2eObjectTypes.LIGHTING);
	}
	
	public Cardio2eLightingTransaction(short objectNumber) {  //Simple lighting GET transaction constructor.
		setTransactionType (Cardio2eTransactionTypes.GET);
		super.setObjectType (Cardio2eObjectTypes.LIGHTING);
		setObjectNumber(objectNumber);
	}
	
	public Cardio2eLightingTransaction(short objectNumber, byte lightIntensity) {  //Simple SET transactionType constructor for lighting intensity control.
	...
	public Cardio2eLightingTransaction(short objectNumber, boolean lightPowered) {  //Simple SET transactionType constructor for lighting power control.
		setTransactionType (Cardio2eTransactionTypes.SET);
		super.setObjectType (Cardio2eObjectTypes.LIGHTING);
		setObjectNumber(objectNumber);
	...
	public void setLightIntensity(byte lightIntensity) {
		isDataComplete = false;
		if ((lightIntensity >= CARDIO2E_MIN_LIGHTING_INTENSITY) && (lightIntensity <= CARDIO2E_MAX_LIGHTING_INTENSITY)) {
	...
	public String toString () {
		...
		if (isDataVerified()) {
			...
			case INFORMATION:
			case SET:
				returnString = String.format("%s%s %s %d %d%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,getObjectNumber(),lightIntensity,CARDIO2E_END_TRANSACTION_CHARACTER);
				break;
			}			
		}
		return returnString;
	...
	public boolean smartSendingCanReplaceLikeMe() {
		//This function has been replaced in this Cardio2eTransaction subclass in order to return true.
