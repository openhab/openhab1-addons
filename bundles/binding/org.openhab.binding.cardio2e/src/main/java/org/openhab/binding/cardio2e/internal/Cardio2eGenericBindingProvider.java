...
public class Cardio2eGenericBindingProvider extends AbstractGenericBindingProvider implements Cardio2eBindingProvider {
	...
	public class Cardio2eBindingConfigItem {
		...
		public Cardio2eTransaction transaction; //The Cardio2eTransaction object for send data to Cardio 2è
		public boolean reverseMode = false; //Set reverseMode = true to enable Cardio 2è control only (updates will be sent to Cardio 2è instead commands, and Cardio 2è received updates will be sent as commands to openHab bus instead states)
		public long lastStateSentTimestamp; //Used for store a timestamp of the last state sent 
		private int pendingUpdates = 0; //Used for prevent updates sent to Cardio 2è causes command sending back to bus when reverseMode = true 
