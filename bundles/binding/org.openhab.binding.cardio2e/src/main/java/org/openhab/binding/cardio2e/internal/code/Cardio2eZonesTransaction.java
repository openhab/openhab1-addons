...
public class Cardio2eZonesTransaction extends Cardio2eTransaction {
	...
	public Cardio2eZonesTransaction(short objectNumber) {  //Simple zones GET transaction constructor.
	...
	public void setObjectNumber(short objectNumber) {
		...
		if (objectNumber <= CARDIO2E_MAX_SECURITY_ZONE_NUMBER) {
			...
			zoneStates = null; //objectNumber change must destroy zoneStates array, because zoneStates array length is relative to objectNumber 
		}
		else{
	...
	public void setZoneStates(Cardio2eZoneStates zoneStates[]) {
		...
		if (zoneStates != null) {
			if ((zoneStates.length >= 1) && (zoneStates.length <= maxStatesLenght)) {
				...
				throw new IllegalArgumentException("invalid zoneStates length '" + zoneStates.length + "' or objectNumber not set");
			}
		} else {
			throw new IllegalArgumentException("zoneStates can not be null");			
		}
	}
	...
	public void setZoneState(Cardio2eZoneStates zoneState) {
		...
		if (zoneState != null) {
			...
			throw new IllegalArgumentException("zoneState can not be null");			
		}
	}

	public boolean[] getZoneDetections() {  //Returns boolean array with zones detection data based on on zoneStates[] && zonesMask[] matches. 
	...
	public boolean getZoneDetection() { //Returns boolean value with objectNumber zone detection data based on based on zoneStates[] && zonesMask[] matches.
		return getZoneDetection(getObjectNumber());
	}

	public boolean getZoneDetection(short zoneNumber) { //Returns boolean value with specified zoneNumber zone detection data based on zoneStates[] && zonesMask[] matches.
		if ((zoneStates != null) && (zonesMask != null)) {
			boolean zoneDetection = false;
			short objectNumber = getObjectNumber();
			if ((zoneNumber >= CARDIO2E_MIN_OBJECT_NUMBER)) {
				if ((zoneNumber >= objectNumber) && (zoneNumber < (objectNumber + zoneStates.length))) { //Checks zone range
					zoneDetection = (zoneStates[(zoneNumber-objectNumber)] != zonesMask.getZonesMask()[(zoneNumber-1)]);
