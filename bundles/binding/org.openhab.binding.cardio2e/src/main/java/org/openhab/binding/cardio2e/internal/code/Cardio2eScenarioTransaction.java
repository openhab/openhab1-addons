...
public class Cardio2eScenarioTransaction extends Cardio2eTransaction {
	...
	public Cardio2eScenarioTransaction(short objectNumber) { //Simple SET (execute) scenario transaction constructor.
		setTransactionType(Cardio2eTransactionTypes.SET);
		super.setObjectType (Cardio2eObjectTypes.SCENARIO);
		setObjectNumber(objectNumber);
	}

	public Cardio2eScenarioTransaction(short objectNumber, String securityCode) { //Simple SET (execute) scenario with security code transaction constructor.
		setTransactionType(Cardio2eTransactionTypes.SET);
		super.setObjectType (Cardio2eObjectTypes.SCENARIO);
		setObjectNumber(objectNumber);
		...
		if (isDataVerified()) {
			...
			case SET:
				if (securityCode == null){
					...
					returnString = String.format("%s%s %s %d %s%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,getObjectNumber(),securityCode,CARDIO2E_END_TRANSACTION_CHARACTER);					
				}
				break;
			}			
		}
