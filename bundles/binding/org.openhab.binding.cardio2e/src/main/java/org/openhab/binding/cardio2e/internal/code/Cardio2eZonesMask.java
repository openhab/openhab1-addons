...
public class Cardio2eZonesMask {
	...
	public Cardio2eZonesMask() {
		...
		Arrays.fill(zonesMask,CARDIO2E_DEFAULT_NO_DETECTION_ZONE_STATE); //Inits zones mask		
	}
	...
	public void setZonesMask(Cardio2eZoneStates zonesMask[]) {
		if (zonesMask != null) { //Checks no null
			if (zonesMask.length == Cardio2eTransaction.CARDIO2E_MAX_SECURITY_ZONE_NUMBER) { //Checks array length
				boolean hasNull = false;
				for (Cardio2eZoneStates s: zonesMask) { //Checks no null array elements
	...
	public Cardio2eZoneStates getZoneMask(byte zoneNumber) {
		...
		if ((zoneNumber >= Cardio2eTransaction.CARDIO2E_MIN_OBJECT_NUMBER) && (zoneNumber <= Cardio2eTransaction.CARDIO2E_MAX_SECURITY_ZONE_NUMBER)) {
