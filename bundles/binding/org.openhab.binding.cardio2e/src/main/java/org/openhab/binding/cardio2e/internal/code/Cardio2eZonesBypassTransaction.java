...
public class Cardio2eZonesBypassTransaction extends Cardio2eTransaction{
	...
	public Cardio2eZonesBypassTransaction(short objectNumber) {  //Simple zone bypass GET transaction constructor.
		setTransactionType (Cardio2eTransactionTypes.GET);
		super.setObjectType (Cardio2eObjectTypes.ZONES_BYPASS);
		setObjectNumber(objectNumber);
	}
	
	public Cardio2eZonesBypassTransaction(short objectNumber, Cardio2eZoneBypassStates zoneBypassStates[]) {  //Simple SET transactionType constructor for zone bypass.
	...
	public Cardio2eZonesBypassTransaction(short objectNumber, boolean zoneBypassStates[]) {  //Simple SET transactionType constructor for zone bypass.
		setTransactionType (Cardio2eTransactionTypes.SET);
		super.setObjectType (Cardio2eObjectTypes.ZONES_BYPASS);
		setObjectNumber(objectNumber);
	...
	public void setObjectNumber(short objectNumber) {
		...
		if (objectNumber <= CARDIO2E_MAX_SECURITY_ZONE_NUMBER) {
			...
			zoneBypassStates = null; //objectNumber change must destroy zoneBypassStates array, because zoneStates array length is relative to objectNumber 
		}
		else{
	...
	public boolean[] getZoneBypassBooleanStates() {
		...
		if (zoneBypassStates != null) {
			...
			if ((length >= 1) && (length <= maxStatesLenght)) {
				...
				throw new IllegalArgumentException("invalid zoneBypassStates length '" + zoneBypassStates.length + "' or objectNumber not set");
			}
		} else {
			throw new IllegalArgumentException("can not parse null zoneBypassStates values");
		}	
	...
	public void setZoneBypassStates(Cardio2eZoneBypassStates zoneBypassStates[]) {
		...
		if (zoneBypassStates != null) {
			...
			throw new IllegalArgumentException("zoneBypassStates can not be null");			
		}
	}
	...
	public boolean getZoneBypassBooleanState() {
		...
		if (zoneBypassStates[0] != null) {
			...
			throw new IllegalArgumentException("can not parse null zoneBypassState value");
		}
	...
	public void setZoneBypassState(Cardio2eZoneBypassStates zoneBypassState) {
		...
		if (zoneBypassState != null) {
			if (getObjectNumber() >= CARDIO2E_MIN_OBJECT_NUMBER) {
				...
				throw new IllegalArgumentException("can not set zoneState because objectNumber is not properly defined");
			}
		} else {
			throw new IllegalArgumentException("zoneBypassState can not be null");
		}
	}

	public void setZoneBypassBooleanState(boolean zoneBypassState) {
		if (zoneBypassState) {
			setZoneBypassState((invertZoneBypassBooleanStates) ? Cardio2eZoneBypassStates.NOT_BYPASSED : Cardio2eZoneBypassStates.BYPASSED);						
		}
		else {
			setZoneBypassState((invertZoneBypassBooleanStates) ? Cardio2eZoneBypassStates.BYPASSED : Cardio2eZoneBypassStates.NOT_BYPASSED);													
		}
	}
		...
		if (super.isDataVerified()) {
			...
			case SET:
				if ((getObjectNumber() != -1) && (zoneBypassStates != null)) {
					verified = true; //Verified will be true, excepts null value in array is found.
	...
	public String toString () {
		...
		if (isDataVerified()) {
			...
			case SET:
				...
				returnString = String.format("%s%s %s %d %s%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,getObjectNumber(),returnString,CARDIO2E_END_TRANSACTION_CHARACTER);
				break;
			}			
		}
		return returnString;
