...
/**
 * Cardio2e Binding structured curtain transaction model (only for newest Cardio2e versions). 
 * 
...
public class Cardio2eCurtainTransaction extends Cardio2eTransaction{
	...
	private byte openingPercentage = -1; // -1=Not set, signaling openingPercentage not set.
	...
	public Cardio2eCurtainTransaction(short objectNumber) {  //Simple curtain GET transaction constructor.
		setTransactionType (Cardio2eTransactionTypes.GET);
		super.setObjectType (Cardio2eObjectTypes.CURTAIN);
		setObjectNumber(objectNumber);
	}
	
	public Cardio2eCurtainTransaction(short objectNumber, byte openingPercentage) {  //Simple SET transactionType constructor for curtain opening percentage control.
	...
	public Cardio2eCurtainTransaction(short objectNumber, boolean curtainOpen) {  //Simple SET transactionType constructor for simple curtain opening control.
		setTransactionType (Cardio2eTransactionTypes.SET);
		super.setObjectType (Cardio2eObjectTypes.CURTAIN);
		setObjectNumber(objectNumber);
	...
	public void setOpeningPercentage(byte openingPercentage) {
		isDataComplete = false;
		if ((openingPercentage >= CARDIO2E_MIN_CURTAIN_OPENING_PERCENTAGE) && (openingPercentage <= CARDIO2E_MAX_CURTAIN_OPENING_PERCENTAGE)) {
	...
	public String toString () {
		...
		if (isDataVerified()) {
			...
			case INFORMATION:
			case SET:
				returnString = String.format("%s%s %s %d %d%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,getObjectNumber(),openingPercentage,CARDIO2E_END_TRANSACTION_CHARACTER);
				break;
			}			
		}
		return returnString;
