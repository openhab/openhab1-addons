...
public class Cardio2eRelayTransaction extends Cardio2eTransaction{
	...
	public Cardio2eRelayTransaction(short objectNumber) {  //Simple relay GET transaction constructor.
		setTransactionType (Cardio2eTransactionTypes.GET);
		super.setObjectType (Cardio2eObjectTypes.RELAY);
		setObjectNumber(objectNumber);
	}
	
	public Cardio2eRelayTransaction(short objectNumber, Cardio2eRelayStates relayState) {  //Simple SET transactionType constructor for relay control.
	...
	public Cardio2eRelayTransaction(short objectNumber, boolean relayON) {  //Simple SET transactionType constructor for simple relay control (on=opened)
		setTransactionType (Cardio2eTransactionTypes.SET);
		super.setObjectType (Cardio2eObjectTypes.RELAY);
		setObjectNumber(objectNumber);
		setRelayON(relayON);
	...
	public void setBlindUpMode(boolean enableBlindUpMode) {
		if ((blindDownMode) && (enableBlindUpMode)) blindDownMode = false;
	...
	public void setBlindDownMode(boolean enableBlindDownMode) {
		if ((blindUpMode) && (enableBlindDownMode)) blindUpMode = false;
	...
	public String toString () {
		...
		if (isDataVerified()) {
			...
			case INFORMATION:
			case SET:
				returnString = String.format("%s%s %s %d %s%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,getObjectNumber(),relayState.symbol,CARDIO2E_END_TRANSACTION_CHARACTER);
				break;
			}			
		}
		return returnString;
