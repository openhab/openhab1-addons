...
public class Cardio2eSecurityTransaction extends Cardio2eTransaction {
	...
	public Cardio2eSecurityTransaction(Cardio2eTransactionTypes transactionType) { //Simple security transactionType constructor, suitable for simple GET transactions use.
		setTransactionType(transactionType);
		super.setObjectType (Cardio2eObjectTypes.SECURITY);
		setObjectNumber((byte)1);
	}

	public Cardio2eSecurityTransaction(Cardio2eSecurityStates securityState, String securityCode) { //Simple SET security arm / disarm transaction constructor.
	...
	public String toString () {
		...
		if (isDataVerified()) {
			...
			case NACK:
				...
				break;
			case INFORMATION:
				returnString = String.format("%s%s %s %d %s%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,getObjectNumber(),securityState.symbol,CARDIO2E_END_TRANSACTION_CHARACTER);
			case SET:
				returnString = String.format("%s%s %s %d %s %s%s",CARDIO2E_START_TRANSACTION_INITIATOR,getTransactionType().symbol,getObjectType().symbol,getObjectNumber(),securityState.symbol,securityCode,CARDIO2E_END_TRANSACTION_CHARACTER);
				break;
			}			
		}
		return returnString;
