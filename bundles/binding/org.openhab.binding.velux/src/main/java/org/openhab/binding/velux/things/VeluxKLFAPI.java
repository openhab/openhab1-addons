...
/**
 * <B>Velux</B> KLF200 API representation.
 * <P>
 * Enumeration of available API message identifiers.
 * <P>
...
public class VeluxKLFAPI {
	...
	/**
	 * Enumeration of complete API of the {@link VeluxKLFAPI}.
	 * <P>Methods available:
	 * <UL>
	...
	public enum Command {
		...
		GW_UNDEFCOMMAND(					(short) -1,	 	"Unknown command."),
		...
		GW_SET_FACTORY_DEFAULT_REQ(			(short) 0x0003, "Request gateway to clear system table, scene table and set Ethernet settings to factory default. Gateway will reboot."),
		...
		GW_CS_GET_SYSTEMTABLE_DATA_NTF(		(short) 0x0102, "Acknowledge to GW_CS_GET_SYSTEM_TABLE_DATA_REQList of nodes in the gateways systemtable."),
		...
		GW_CS_CONTROLLER_COPY_REQ(			(short) 0x010A, "Setup KLF200 to get or give a system to or from another io-homecontrol®  remote control. By a system means all nodes in the systemtable and the system key."),
		...
		GW_CS_PGC_JOB_NTF(					(short) 0x0111, "Information on Product Generic Configuration job initiated by  press on PGC button."),
		GW_CS_SYSTEM_TABLE_UPDATE_NTF(		(short) 0x0112, "Broadcasted to all clients and gives information about added and removed actuator nodes in system table."),
		...
		GW_CS_ACTIVATE_CONFIGURATION_MODE_REQ((short) 0x0119, "Request one or more actuator to open for configuration."),
		...
		GW_GET_ALL_NODES_INFORMATION_NTF(	(short) 0x0204, "Acknowledge to GW_GET_ALL_NODES_INFORMATION_REQ. Holds node information"),
		GW_GET_ALL_NODES_INFORMATION_FINISHED_NTF(			(short) 0x0205, "Acknowledge to GW_GET_ALL_NODES_INFORMATION_REQ. No more nodes."),
		...
		GW_GROUP_INFORMATION_CHANGED_NTF(	(short) 0x0224, "Broadcast to all, about group information of a group has been changed."),
		...
		GW_GROUP_DELETED_NTF(				(short) 0x022D, "GW_GROUP_DELETED_NTF is broadcasted to all, when a group has been removed."),
		...
		GW_COMMAND_REMAINING_TIME_NTF(		(short) 0x0303, "Gives remaining time before io-homecontrol®  node enter target position."),
		GW_SESSION_FINISHED_NTF(			(short) 0x0304, "Command send, Status request, Wink, Mode or Stop session is finished."),
		...
		GW_STATUS_REQUEST_NTF(				(short) 0x0307, "Acknowledge to GW_STATUS_REQUEST_REQ. Status request from one or more io-homecontrol®  nodes."),
		...
		GW_GET_CONTACT_INPUT_LINK_LIST_REQ(	(short) 0x0460, "Get list of assignments to all Contact Input to scene  or product  group."),
		...
		GW_GET_LOCAL_TIME_REQ(				(short) 0x2004, "Request  the local time based on current time zone and daylight savings rules."),
		...
		GW_PASSWORD_CHANGE_NTF(				(short) 0x3004, "Acknowledge to GW_PASSWORD_CHANGE_REQ. Broadcasted to all connected clients."),
		...
		Command(String categoryString) {
			try {
				...
				try {
					this.command = Command.valueOf(categoryString.replaceAll("\\p{C}", "_").toUpperCase())
							.getCommand();
		...
		public static Command get(int typeId) {
