...
public class OnewireCunoBinding extends AbstractBinding<OnewireCunoBindingProvider>
	 ...
	 * H4A7E81380000FF -> 0038 -> -3,8 
	 * H4A7E81010000FF -> 0001 -> -0,1
	 * H7C6E01500800FF -> 0800 -> 80,0
	 * @param message
	 */
	private void handleReceivedMessage(String message) {
		...
		if (config != null) {
			State state = null;
			if (message.substring(5, 11).equals("810100")) {
				logger.debug("value for {} not available", config.getAddress());
				state = UnDefType.UNDEF;
			} else if (message.substring(5, 11).equals("015008")) {
				logger.debug("device {} not available", config.getAddress());
				state = UnDefType.UNDEF;
			} else {
				float converted = 0;
				// switch two bytes
				...
				converted /= 10;
				if (message.substring(5, 6).startsWith("8")) {
