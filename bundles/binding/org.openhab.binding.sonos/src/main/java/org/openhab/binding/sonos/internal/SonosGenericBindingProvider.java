...
/**
 * 
 * sonos commands will be limited to the simple commands that take only up to
 * one input parameter. unpnp actions requiring input variables could
 * potentially take their inputs from elsewhere in the binding, e.g. config
 * parameters or other
 * 
 * sonos="[ON:office:play], [OFF:office:stop]" - switch items for ordinary sonos
 * commands using openhab command : player name : sonos command as format
 * 
 * sonos="[office:getcurrenttrack]" - string and number items for UPNP service
 * variable updates using using player_name : somecommand, where somecommand
 * takes a simple input/output value from/to the string
 * 
...
public class SonosGenericBindingProvider extends AbstractGenericBindingProvider
	...
	private static final Pattern ACTION_CONFIG_PATTERN = Pattern
			.compile("\\[(.*):(.*):(.*)\\]");
	private static final Pattern STATUS_CONFIG_PATTERN = Pattern
			.compile("\\[(.*):(.*)\\]");
	...
	private void parseAndAddBindingConfig(Item item, String bindingConfigs)
			throws BindingConfigParseException {
	...
	private void parseBindingConfig(SonosBindingConfig config, Item item,
		...
		if (bindingConfig != null) {
			...
			if (!actionMatcher.matches() && !statusMatcher.matches()) {
				...
				SonosBindingConfigElement newElement = new SonosBindingConfigElement(
						sonosCommand, sonosID, item.getAcceptedDataTypes());

				Command command = null;
				if (commandAsString == null) {
					command = createCommandFromString(null,
	 ...
	 * 
	 * @param item
	 *            , or null if the Command has to be of the StringType type
	...
	private Command createCommandFromString(Item item, String commandAsString)
		...
		Command command = TypeParser.parseCommand(acceptedTypes,
				commandAsString);
	 ...
	 */
	static class SonosBindingConfig extends
			HashMap<Command, SonosBindingConfigElement> implements
			BindingConfig {
	...
	public List<String> getSonosID(String itemName) {
		...
		SonosBindingConfig aConfig = (SonosBindingConfig) bindingConfigs
				.get(itemName);
	...
	public String getSonosID(String itemName, Command aCommand) {
		SonosBindingConfig aConfig = (SonosBindingConfig) bindingConfigs
				.get(itemName);
		return aConfig != null && aConfig.get(aCommand) != null ? aConfig.get(
				aCommand).getSonosID() : null;
	}
	...
	public List<Class<? extends State>> getAcceptedDataTypes(String itemName) {
		...
		if (aConfig != null) {
			if (aConfig.size() != 0) {
				Iterator<SonosBindingConfigElement> it = aConfig.values()
						.iterator();
				while (it.hasNext()) {
	...
	public String getSonosCommand(String itemName, Command aCommand) {
		SonosBindingConfig aBindingConfig = (SonosBindingConfig) bindingConfigs
				.get(itemName);
		if (aBindingConfig != null) {
			for (Command command : aBindingConfig.keySet()) {
				SonosBindingConfigElement anElement = aBindingConfig
						.get(command);
				if (command == aCommand) {
		...
		for (String anItem : items) {
			...
			for (Command command : aBindingConfig.keySet()) {
				...
				if (anElement.getSonosCommand().equals(sonosCommand)
						&& anElement.getSonosID().equals(sonosID)
	...
	public List<Command> getVariableCommands(String anItem) {
		...
		for (Command aCommand : aConfig.keySet()) {
			if (aCommand instanceof StringType
					|| aCommand instanceof DecimalType) {
				commands.add(aCommand);
			}
		}
		return commands;
