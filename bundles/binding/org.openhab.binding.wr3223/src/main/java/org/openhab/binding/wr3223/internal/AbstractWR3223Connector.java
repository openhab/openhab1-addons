...
public abstract class AbstractWR3223Connector {
	...
	private static final Logger logger = 
			LoggerFactory.getLogger(AbstractWR3223Connector.class);
		...
		if(answer[0] == STX && answer[answer.length-1] == ETX ){
			if(chkSum == buildCheckSum(answer,1, answer.length-1)){
				if(command.name().equals(toString(answer,1,2))){
					...
					logger.error("Wrong command receive. Expected {} but got {}.",command.name(), toString(answer,1,2));
				}
			}else{
				logger.error("Checksum error. Expected {} but got {}.",chkSum, buildCheckSum(answer,1, answer.length-1));
			}
		}else{
