...
public class PowerMaxBinding extends AbstractActiveBinding<PowerMaxBindingProvider>
    ...
    public void updated(Dictionary<String, ?> config) throws ConfigurationException {
        ...
        if (config == null) {
            logger.warn("PowerMax alarm binding: missing configuration");
            this.setProperlyConfigured(false);
            throw new ConfigurationException(null, "missing configuration");
        }
        ...
        if (StringUtils.isNotBlank((String) config.get("forceStandardMode"))) {
            ...
            PowerMaxReceiveType.POWERLINK
                    .setHandlerClass(forceStandardMode ? PowerMaxBaseMessage.class : PowerMaxPowerlinkMessage.class);
        }
    ...
    public void powerMaxEventReceived(EventObject event) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("powerMaxEventReceived(): received message {}", (message.getReceiveType() != null)
                    ? message.getReceiveType().getDescription() : String.format("%02X", message.getCode()));
        }
