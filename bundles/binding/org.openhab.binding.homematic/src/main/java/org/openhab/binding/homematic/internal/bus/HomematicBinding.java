...
public class HomematicBinding extends AbstractActiveBinding<HomematicBindingProvider> implements ManagedService,
	...
	protected void internalReceiveCommand(String itemName, Command command) {
		...
		for (HomematicBindingProvider provider : providers) {
			...
			if (provider.isAdminItem(itemName)) {
				...
				CommandConverter<?, ?> commandConverter = converterLookup.getCommandToBindingValueConverter(itemName,
						command.getClass());
	...
	private synchronized void removeCallbackHandler() {
		...
		try {
			...
			logger.debug("Error while unregistering callback server. Will be ignored.", e);
		}
		cbServer = null;
	}
