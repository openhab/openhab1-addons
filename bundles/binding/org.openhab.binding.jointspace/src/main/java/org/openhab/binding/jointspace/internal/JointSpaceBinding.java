 ...
 * @since 1.5.0
 */
public class JointSpaceBinding extends
		AbstractActiveBinding<JointSpaceBindingProvider> implements
		ManagedService {

	private static final Logger logger = LoggerFactory
			.getLogger(JointSpaceBinding.class);
	...
	protected void execute() {
		...
		try {
			...
			if (success) {
				logger.debug(
						"established connection [host '{}' port '{}' timeout '{}']",
				...
				for (JointSpaceBindingProvider provider : providers) {
					for (String itemName : provider.getItemNames()) {
						String tvcommand = provider.getTVCommand(itemName,
								"POLL");
						if (tvcommand != null) {
				...
				logger.debug(
						"couldn't establish network connection [host '{}' port '{}' timeout '{}']",
						new Object[] { ip, 0, timeout });
			}
		} catch (SocketTimeoutException se) {
			logger.debug(
					"timed out while connecting to host '{}' port '{}' timeout '{}'",
					new Object[] { ip, 0, timeout });
	...
	private String[] command2LayerString(String command) {
		...
		if (temp.length > 1) {
			...
			layer[layer.length - 1] = layer[layer.length - 1].replace(']', ' ')
					.trim();
		} else {
			layer = null;
		}
	...
	private void updateItemState(String itemName, String tvCommand) {
		if (tvCommand.contains("ambilight")) {
			...
			HSBType state = new HSBType(getAmbilightColor(ip + ":" + port,
					layer));
			eventPublisher.postUpdate(itemName, state);
		} else if (tvCommand.contains("volume")) {
			if (tvCommand.contains("mute")) {
				eventPublisher.postUpdate(itemName,
						getTVVolume(ip + ":" + port).mute ? OnOffType.ON
								: OnOffType.OFF);
			} else {
				eventPublisher.postUpdate(itemName, new DecimalType(
						getTVVolume(ip + ":" + port).volume));
			}
		} else if (tvCommand.contains("source")) {
			eventPublisher.postUpdate(itemName, new StringType(getSource(ip
					+ ":" + port)));
		} else {
			logger.error("Could not parse item state\"" + tvCommand
					+ "\" for polling");
			return;
		}
	...
	protected void internalReceiveCommand(String itemName, Command command) {
		...
		if (itemName != null) {
			JointSpaceBindingProvider provider = findFirstMatchingBindingProvider(
			...
			if (tvCommandString.contains("key")) {
				...
				if (commandlist.length != 2) {
					logger.warn(
							"wrong number of arguments for key command \"{}\". Should be key.X",
							tvCommandString);
					return;
				}
				...
				if (commandlist.length < 2) {
					logger.warn(
							"wrong number of arguments for source command \"{}\". Should be at least mode.X...",
							tvCommandString);
					return;
				}
				...
				logger.warn(
						"Unrecognized tv command \"{}\". Only key.X or ambilight[].X is supported",
						tvCommandString);
				return;
			}
		}
	...
	private Color getAmbilightColor(String host, String[] layers) {
		...
		logger.debug("Getting ambilight color for host {} for layers {}", host,
				layers);
		Color retval = new Color(0, 0, 0);
		String url = "http://" + host + "/1/ambilight/processed";

		String ambilight_json = HttpUtil.executeUrl("GET", url,
				IOUtils.toInputStream(""), CONTENT_TYPE_JSON, 1000);
		if (ambilight_json != null) {
			logger.trace("TV returned for ambilight request: {}",
					ambilight_json);
			try {
				...
				for (String layer : layers) {
					...
					if (array == null) {
						logger.warn(
								"Could not find layer {} in the json string",
								layer);
						return null;
					}
				}
				...
				logger.warn(
						"Could not parse JSON String for ambilight value. Error: {}",
						t.toString());
			}

		} else {
			logger.debug(
					"Could not get ambilight value from JointSpace Server \"{}\"",
					host);
			return null;
		}
	...
	private String getSource(String host) {
		...
		String source_json = HttpUtil.executeUrl("GET", url,
				IOUtils.toInputStream(""), CONTENT_TYPE_JSON, 1000);
		...
		if (source_json != null) {
			...
			try {
				...
				logger.warn(
						"Could not parse JSON String for source. Error: {}",
						t.toString());
			}

		} else {
			logger.debug("Could not get source from JointSpace Server \"{}\"",
					host);
		}
		return null;
	}
	...
	private void sendSource(String host, String source) {
		...
		HttpUtil.executeUrl("POST", url, IOUtils.toInputStream(content),
				CONTENT_TYPE_JSON, 1000);
	}
	...
	private void sendVolume(String host, Command command) {
		...
		logger.debug("Sending volume to host {} for command {}", host,
				command.toString());
		...
		String content = "{\"muted\":\"" + conf.mute + "\", \"current\":\""
				+ newvalue + "\"}";
	...
	private void setAmbilightColor(String host, Command command, String[] layers) {
		...
		logger.debug("Setting Ambilight color for host {} and layer {} to {}",
				host, layers, command.toString());
		...
		logger.trace("Trying to post json for ambilight: {}",
				content.toString());

		HttpUtil.executeUrl("POST", url,
				IOUtils.toInputStream(content.toString()), CONTENT_TYPE_JSON,
				1000);
	}
	...
	private volumeConfig getTVVolume(String host) {
		...
		String url = "http://" + host + "/1/audio/volume";
		String volume_json = HttpUtil.executeUrl("GET", url,
				IOUtils.toInputStream(""), CONTENT_TYPE_JSON, 1000);
		if (volume_json != null) {
			try {
				...
				logger.warn(
						"Could not parse JSON String for volume value. Error: {}",
						t.toString());
			}
