...
/**
 * This represents the configuration of a openHAB item that is binded to a
 * Mi-Light bulb. It contains the following information:
 * 
 * <ul>
 * <li>The channel number the bulb has on the Milight bridge where 0 represents
 * all white bulbs, 1-4 white channels an 5 RGB bulbs.</li>
...
public class WifiLightBindingConfig extends HashMap<String, String> implements
	...
	public enum BindingType {
		brightness, colorTemperature, rgb, whiteMode, blue
	}
	 ...
	 * 
	 * @param deviceNumber
	 *            The number under which the bulb is filed in the Mi-Light
	 *            bridge.
	 * @param type
	 ...
	 * @throws BindingConfigParseException
	 */
	private int parseChannelNumberConfigString(String configString)
			throws BindingConfigParseException {
		try {
			return Integer.parseInt(configString);
		} catch (Exception e) {
			throw new BindingConfigParseException(
					"Error parsing channel number.");
		}
	}
	 ...
	 * @throws BindingConfigParseException
	 */
	private int parseStepsConfigString(String configString)
			throws BindingConfigParseException {
		try {
			return Integer.parseInt(configString);
		} catch (Exception e) {
	...
	/**
	 * @return The number of dimming steps as a
	 *         {@link WifiLightBindingConfig.BindingType} that has been declared
