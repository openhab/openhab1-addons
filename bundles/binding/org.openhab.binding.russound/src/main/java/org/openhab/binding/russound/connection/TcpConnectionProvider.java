...
public class TcpConnectionProvider implements ConnectionProvider {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(TcpConnectionProvider.class);
	...
	public boolean connect() {
		...
		if (eiscpSocket == null || !connected || !eiscpSocket.isConnected()) {
			try {
				...
				eiscpSocket.connect(new InetSocketAddress(receiverIP,
						receiverPort), CONNECTION_TIMEOUT);

				logger.debug("Connected to {} on port {}", receiverIP,
						receiverPort);

				// Get Input and Output streams
				outStream = new ObjectOutputStream(
						eiscpSocket.getOutputStream());
