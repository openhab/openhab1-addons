...
(function (factory) {
	if (typeof define === 'function' && define.amd) {
		// AMD
		//define(['jquery','ext','openhab'], factory);
		factory(jQuery, Ext, OpenHAB);
	} else {
		// Browser globals, Window
		factory(jQuery, Ext, OpenHAB);
	}
}(function (jQuery, Ext, OpenHAB) {
...
// ------------------------------------
	...
	function updateWidgets(result) {
		...
		if (Ext.isArray(result.widget)) {
			Ext.each(result.widget, function (widget, index) {
				//for (var i = 0 in result.widget) {
				if (widget.type == 'Frame') {
					if (Ext.isArray(widget.widget)) {
						Ext.each(widget.widget, function (widget2, index2) {
							setWidgetData(widget2);
						});
					} else {
	...
	var ajax_requests = []; //holds pending ajax requests
	var broadCrumb = []; // Broadcrumb array
	var broadCrumbText = '';
	var newCard;
	...
	function setTheme() {
		...
		var theme_css_filename = './themes/' + current_theme + '/css/style.css';
	...
	var lang = OpenHAB.i18n_strings[ui_language];
	...
	var sitemapsStore;
	sitemapsStore = new Ext.data.Store({
		fields: ['name', 'value'],
		autoLoad: true,
		listeners: {
			exception: function (proxy, response, op, opt) {
				if (sitemapStoreLoadTries > 0) {
					...
					alert(lang.error_server_connection);
				}
			},
			load: function (store, records, success) {
				if (success) {
					Ext.iterate(records, function (record) {
						record.data.value = record.data.name;
					});
					sitemapStoreLoadTries = 3;
					sitemapsStoreSelection.insert(0, [{
						name: '- ' + lang.choose_on_startup,
						value: 'choose'
					}]);
							...
							},
							{
								...
								handler: function () {
									if (dirtySettings) {
										...
										alert(lang.need_to_restart_for_changes_to_take_effect);
		...
		try {
			if (json) {
				...
				if (Ext.isArray(page_data)) {
					var frame = false;
					Ext.each(page_data, function (data, index) {
						if (data.type != 'Frame') {
							if (!frame) {
								container.push({
									xtype: 'fieldset',
								...
								container = container[container.length - 1]['items'] = [];
								frame = true;
							}
							pushWidget(addsWidget(data.widgetId, data, container, nav_parent), container);
							...
							container[container.length - 1]['items'] = [];
							if (Ext.isArray(data.widget)) {
								Ext.each(data.widget, function (widgetD, k) {
									pushWidget(addsWidget(widgetD.widgetId, widgetD, container[container.length - 1]['items'], nav_parent), container[container.length - 1]['items']);
								});
							} else {
								pushWidget(addsWidget(data.widget.widgetId, data.widget, container[container.length - 1]['items'], nav_parent), container[container.length - 1]['items']);
							}
						}
					});
				} else {
					if (page_data.type != 'Frame') {
						...
						if (Ext.isArray(page_data.widget)) {
							Ext.each(page_data.widget,
								function (widget, index) {
									pushWidget(addsWidget(widget.widgetId, widget, container[container.length - 1]['items'], nav_parent), container[container.length - 1]['items']);
								});
						} else {
	...
	function clearEmptyFrames() {
		Ext.iterate(UIobjects, function (key, uIobject) {
			var newArray = [];
			Ext.each(uIobject, function (obj) {
				if (obj.items && obj.items.length > 0) {
					newArray.push(obj);
				}
			});
			UIobjects[key] = newArray;
		});
	}
	...
	function addButtonToLeftNav(id, data, nav_parent) {
		...
		nav_parent.children[nav_parent.children.length - 1].children = [];
	...
	function showSettingsWindow() {
		if (settingsPanel) {
			...
			var options_array = [];
			Ext.iterate(OpenHAB.i18n_strings, function (key, value) {
				options_array.push({
			...
			options_array = [];
			Ext.iterate(OpenHAB.themes, function (key, value) {
				options_array.push({
					text: value,
					value: value
				})
			...
			settingsPanel.getItems().items[0].getItems().items[5].setOptions([
				{text: lang.auto_detect, value: 'auto'},
	...
	function tapHandler(btn, evt) {
		...
		if (deviceType == 'Phone') {
			...
			var leftNav = Ext.getCmp('leftPanel').getActiveItem();
			var index = leftNav.getStore().getAt(leftNav.getStore().find('page_id', btn.config.page_id)).data.index;
	...
	function updateWidgetsAjax(page, update_type) {
		if (page == getCurrentPageId()) {
			...
			Ext.each(ajax_requests, function (request) {
	...
	function updateLeftNavMenuItem(page_id, title, icon) {
		var nav_store = Ext.getCmp('leftPanel').getActiveItem().getStore();
		var index = nav_store.find('page_id', page_id);
		if (index != -1) {
	...
	function updateLeftNavMenu(updatedata) {
		var nav_store = Ext.getCmp('leftPanel').getActiveItem().getStore();
		var index = nav_store.find('name', updatedata.widgetId);
		if (index != -1) {
...
// -------- here we set some variables depending on device type and orientation ----------
	...
	Ext.define('Oph.field.ButtonsSelect', {
		...
		applyComponent: function (config) {
			var _this = this;
			var container = Ext.factory({
				styleHtmlContent: true,
				styleHtmlCls: 'oph_right'
			}, Ext.Container);

			if (Ext.isArray(this.config.options)) {
				Ext.each(this.config.options, function (option, index) {
					var item = container.add(Ext.create('Ext.Button', {
						cls: 'x-button-normal oph_selection_btn',
						item: _this.config.oph_item,
				...
				var item = container.add(Ext.create('Ext.Button', {
					cls: 'x-button-normal oph_selection_btn',
					item: _this.config.oph_item,
					command: _this.config.options.command,
		...
		setValueData: function (newValue) {
			...
			if (Ext.isArray(newValue.mapping)) {
				Ext.each(newValue.mapping,
					function (mapping, index) {
						_this._component._items.items[index].removeCls('x-button-action');
						if (newValue.item.state == newValue.mapping[index].command) {
							_this._component._items.items[index].addCls('x-button-action');
						}
					});
			} else {
		...
		initialize: function () {
			...
			this.on({
				painted: function (component) {
					_this._component.element.on({
						tap: function () {
							sendCommand(_this.config.oph_item, _this.getValue())
						}
					});
					this._component.getThumb(0).element.on({
						dragend: function () {
							sendCommand(_this.config.oph_item, _this.getValue())
						}
					});
		...
		initialize: function () {
			...
			this.on({
				spindown: function (component, value) {
					sendCommand(_this.config.oph_item, value);
				},
		...
		initialize: function () {
			...
			this.on({
				painted: function (component) {
					_this._component.element.on({
						singletap: function () {
							sendCommand(_this.config.oph_item, _this.formatOutputValue(_this.getValue()));
						}
					});
					_this._component.getThumb(0).element.on({
						dragend: function () {
							sendCommand(_this.config.oph_item, _this.formatOutputValue(_this.getValue()))
						}
					});
	...
	Ext.define('Oph.field.Rollershutter', {
		...
		applyComponent: function (config) {
			...
			var container = Ext.factory({
				...
				Ext.Container);
			var but1 = container.add(Ext.create('Ext.Button', {
				cls: 'x-button-normal oph_rollershutter_btn',
				longPress: false,
				iconCls: 'arrow_up',
				iconMask: true
			}));
			but1.element.item = _this.config.oph_item;
			but1.element.on({
				touchstart: function () {
			...
			var but2 = container.add(Ext.create('Ext.Button', {
				cls: 'x-button-normal oph_rollershutter_btn',
				iconCls: 'delete',
				iconMask: true
			}));
			but2.element.item = _this.config.oph_item;
			but2.element.on({
				tap: function () {
					sendCommand(this.item, 'STOP')
				}
			});
			var but3 = container.add(Ext.create('Ext.Button', {
				cls: 'x-button-normal oph_rollershutter_btn',
				longPress: false,
				iconCls: 'arrow_down',
				iconMask: true
			}));
			but3.element.item = _this.config.oph_item;
			but3.element.on({
				touchstart: function () {
				...
				this.on({
					painted: function (component) {
						if (component.config.oph_refresh > 0) {
							component.config.refreshInterval = window.setInterval(
								function () {
									component.updateImage(component);
								},
								component.config.oph_refresh);
						}
					},
	...
	oph_app = Ext.application({
		...
		launch: function () {
			...
			sitemapUIpanel.onAfter('activeitemchange',
				function (container, newcard, oldcard, opts) {
					if (oldcard) {
				...
				sitemapUIpanel);
			sitemapUIpanel.onBefore('activeitemchange',
				function (container, newcard, oldcard, opts) {
						...
						show: function () {
							...
							Ext.each(items.items, function (item) {
								if (left_column_height > right_column_height) {
									right_column_height += item.element.dom.clientHeight;
									item.addCls(deviceTypeCls + '_fieldset_right');
								} else {
									left_column_height += item.element.dom.clientHeight;
									item.addCls(deviceTypeCls + '_fieldset_left');
								}
							});
							//------------------------
						}
			...
			if (deviceType != 'Phone') {
				...
				leftNavPanel.doBack =
					function (me, node, lastActiveList, detailCardActive) {
						broadCrumb.pop();
